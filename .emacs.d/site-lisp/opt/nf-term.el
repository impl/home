...
;; Local variables for reading custom messages through the terminal
(defvar term-ansi-message-next-expected-char 0)
(defvar term-ansi-message-var nil)
(defvar term-ansi-message-buffer nil)
...
Entry to this mode runs the hooks on `term-mode-hook'."
  ...
  (make-local-variable 'term-ansi-message-next-expected-char)
  (make-local-variable 'term-ansi-message-var)
  (make-local-variable 'term-ansi-message-buffer)
...
(defun term-set-default-directory ()
...
(defconst term-ansi-message-string "nSiT"
  "A string representing the ANSI terminal message escape characters used with this terminal.")
	  ...
	  (while (< i str-length)
	    ...
	    (cond ((< term-terminal-state 2)
		  ...
		  ((eq term-terminal-state 2)	  ; Seen Esc
		   (cond ((eq char ?A)            ;; Emacs-specific escape codes
                          (setq term-ansi-message-next-expected-char 0)
                          (setq term-terminal-state 781))
                         ((eq char ?\133)	  ;; ?\133 = ?[
		  ...
		  ((eq term-terminal-state 3) ; Seen Esc [
		   (cond ((and (>= char ?0) (<= char ?9))
			 ...
			 (t
			  ...
			  (setq term-terminal-previous-parameter -1)
			  (setq term-terminal-state 0))))
                  ((eq term-terminal-state 781) ; Seen Esc A
                   (if (eq term-ansi-message-next-expected-char
                           (length term-ansi-message-string))
                       ;; At the end of the escape string, check the character
                       ;; and go to the next state
                       ;;
                       ;; The next character should be either ?u, ?h or ?c
                       (progn
                         (cond ((eq char ?c)
                                (setq term-ansi-message-var 'term-ansi-at-dir))
                               ((eq char ?u)
                                (setq term-ansi-message-var 'term-ansi-at-user))
                               ((eq char ?h)
                                (setq term-ansi-message-var 'term-ansi-at-host))
                               ;; If it isn't, skip until the \0
                               ((setq term-ansi-message-var nil)))
                         (setq term-ansi-message-buffer nil)
                         (setq term-terminal-state 782))
                     ;; Loop through the escape string
                     (let ((requested-char (aref term-ansi-message-string
                                                 term-ansi-message-next-expected-char)))
                       (if (eq char requested-char)
                           (setq term-ansi-message-next-expected-char
                                 (1+ term-ansi-message-next-expected-char))
                         ;; No match -- go back to the initial state
                         ;; This might discard some characters if invalid escape
                         ;; sequences are used, but I don't care
                         (setq term-terminal-state 0)))))
                  ((eq term-terminal-state 782) ; Seen Esc AnSiT?
                   (if (eq char ?\0)
                       (progn
                         (when term-ansi-message-var
                           ;; Note: not setq
                           (set term-ansi-message-var term-ansi-message-buffer)
                           (term-set-default-directory))
                         (setq term-terminal-state 0))
                     (setq term-ansi-message-buffer
                           (concat term-ansi-message-buffer (list char))))))
